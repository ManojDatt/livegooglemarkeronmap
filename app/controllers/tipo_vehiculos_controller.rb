class TipoVehiculosController < ApplicationController
  ...
  def create
    ...
    respond_to do |format|
      if @tipo_vehiculo.save
        format.html { redirect_to tipo_vehiculos_url, notice: 'Tipo vehiculo Se creó correctamente.' }
        format.json { render :index, status: :created, location: @tipo_vehiculo }
      else
  ...
  def update
    respond_to do |format|
      if @tipo_vehiculo.update(tipo_vehiculo_params)
        format.html { redirect_to tipo_vehiculos_url, notice: 'Tipo vehiculo Se ha actualizado correctamente.' }
        format.json { render :index, status: :ok, location: @tipo_vehiculo }
      else
  ...
  def destroy
    ...
    respond_to do |format|
      format.html { redirect_to tipo_vehiculos_url, notice: 'Tipo vehiculo Fue destruido con éxito.' }
  ...
  private
    ...
    def tipo_vehiculo_params
      params.require(:tipo_vehiculo).permit(:PKTIPO_AUTO, :PKCATEGORIA, :TAUT_DESCRIPCION)
    end
end
